...
pub fn update_people(
    ...
    if people.len() < config.max_pedestrians && frame_count % 15 == 0 {
        ...
        people.push(Person {
            x: start_x,
            speed: rng.gen_range(0.1..0.3) * dir,
            color: theme.pedestrian,
            id_offset: rng.gen_range(0..100),
            is_entering: false,
            entry_pause_timer: 0,
        });
    }
        ...
        if !p.is_entering {
            ...
            if near_door && rng.gen_bool(0.02) {
                ...
                if rng.gen_bool(chance) {
                    p.is_entering = true;
                    p.entry_pause_timer = 60;
                }
            }
        } else {
            if p.entry_pause_timer > 0 {
                p.entry_pause_timer -= 1;
            } else {
                return false;
            }
        }
