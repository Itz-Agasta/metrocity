...
pub fn update_vehicles(
    ...
    // Target 10% of max vehicles, modulated by pulse
    let base_targets = c_max * 0.10;
    ...
    if vehicles.len() < target_vehicles && frame_count % 3 == 0 {
        ...
        if roll < 0.02 && *chase_cooldown == 0 {
            ...
            vehicles.push(Vehicle {
                ...
                y,
                speed,
                color: theme.police_red,
                v_type: VehicleType::Spinner,
                length: 3,
            });
            vehicles.push(Vehicle {
                x: -15.0,
            ...
            *chase_cooldown = 1200;
            ...
            let mega_count = vehicles
                .iter()
                .filter(|v| v.v_type == VehicleType::Shuttle && v.length > 9)
                .count();
            let adj_length = length.min(25);
    ...
    let speed_mod = 0.5 * config.vehicle_speed_multiplier;
    vehicles.retain_mut(|v| {
        v.x += v.speed * speed_mod;
        v.x < (area.width as f32 + 40.0)
    });
}
